...
Authors: Sikchi, Harshit and Zhou, Wenxuan and Held, David
Code available at: https://github.com/hari-sikchi/LBPO
...
def LBPO(env_fn, env_name = '', actor_critic = None, ac_kwargs=dict(), seed=0, 
        steps_per_epoch=4000, epochs=50, gamma=0.99, clip_ratio=0.2, pi_lr=3e-4,
        vf_lr=1e-3, jf_lr=1e-3, penalty_init=1., penalty_lr=5e-2, cost_lim=25, train_pi_iters=80, train_v_iters=80, lam=0.97, target_kl=0.01, target_l2=0.012, logger_kwargs=dict(), save_freq=10, beta=0.01, beta_thres=0.05):
    """
    ...
    o, _ = env.reset()
    ep_ret,ep_cost_ret, ep_len = 0, 0, 0
    ep_cum_violations = 0
    total_cum_violations = 0
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            if terminated or epoch_ended:
                ...
                # if trajectory didn't reach terminated state, bootstrap value target
                ...
                if terminated:
                    ...
                    logger.store(EpRet=ep_ret, EpCostRet=ep_cost_ret, EpLen=ep_len, EpCumViolations=ep_cum_violations)
                o, _ = env.reset()
                ep_ret , ep_cost_ret, ep_len, ep_cum_violations = 0, 0, 0, 0
        ...
        # Perform PPO update!
        update(epoch, ep_cum_violations, total_cum_violations)
        # Log info about epoch
        logger.log_tabular('Epoch', epoch)
        logger.log_tabular('EpRet', with_min_and_max=True)
        logger.log_tabular('EpCostRet', with_min_and_max=True)
        logger.log_tabular('EpCumViolations', with_min_and_max=True)
        logger.log_tabular('CumViolations', total_cum_violations)
