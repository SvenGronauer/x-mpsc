r"""Classes for constraints and lists of constraints.
...
from x_mpsc.models.symbolic_model import SymbolicModel
...
class Constraint:
    ...
    def __init__(self,
        ...
        Args:
            env (safe_control_gym.envs.bechmark_env.gym.Env): The environment the constraint is for.
...
class QuadraticConstraint(Constraint):
    ...
    def __init__(self,
        ...
        Args:
            model (SymbolicModel): The symbolic model the constraint is for.
        ...
        """
        super().__init__(model, constrained_variable, strict=strict, active_dims=active_dims, tolerance=tolerance)
        ...
        loggers.info(f"Init quadratic constraint: P={P} and b={b}")
...
class BoxConstraint(LinearConstraint):
    ...
    def __init__(self,
        ...
        Args:
            model (SymbolicModel): The model to constrain.
            lower_bounds (np.array or list): Lower bound of constraint.
            upper_bounds (np.array or list): Upper bound of constraint.
            constrained_variable (ConstrainedVariableType): Type of constraint.
            strict (optional, bool): Whether the constraint is violated also when equal to its threshold.
            active_dims (list or int): List specifying which dimensions the constraint is active for.
            tolerance (float): The distance at which is_almost_active(env) triggers.
        ...
        super().__init__(model, A, b, constrained_variable, strict=strict, active_dims=active_dims, tolerance=tolerance)
        self.num_input_constraints = 2 * dim
        self.check_tolerance_shape()
        loggers.info(f"Init Box constraint: A={A} and b={b}")


class DefaultConstraint(BoxConstraint):
    ...
    def __init__(self,
        ...
        Args:
            env (gym.Env): Environment for the constraint.
...
class SymmetricStateConstraint(BoxConstraint):
...
class ConstraintList:
    ...
    def __init__(self,
        ...
        print(f'num_state_constraints: {self.num_state_constraints}')
...
GENERAL_CONSTRAINTS = {
    ...
    'bounded_constraint': BoxConstraint,
    'default_constraint': DefaultConstraint
}


 # Move to benchmark_env.
def create_ConstraintList_from_list(
        constraint_yaml_list: list,
    ...
    Args:
        ...
        model (SymbolicModel): Env for which the constraints will be applied
    """
    constraint_list = []
    for constraint in constraint_yaml_list:
        ...
        constraint_list.append(con_class(model, **cfg))
