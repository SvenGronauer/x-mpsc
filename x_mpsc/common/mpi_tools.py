...
try:
    ...
    msg = "Could not load mpi4py package. Continue and use 1 CPU core."
    warnings.warn(msg)
...
def mpi_fork(
    ...
    if USE_MPI and n > 1 and os.getenv("IN_MPI") is None:
...
def is_root_process() -> bool:
    return True if (not USE_MPI or MPI.COMM_WORLD.Get_rank() == 0) else False


def proc_id():
    """Get rank of calling process."""
    return 0 if not USE_MPI else MPI.COMM_WORLD.Get_rank()
...
def num_procs() -> int:
    ...
    return 1 if not USE_MPI else MPI.COMM_WORLD.Get_size()
...
def mpi_print(*message):
    """Print out message if process is root."""
    print(*message) if proc_id() == 0 else None


def mpi_sum(x):
    return mpi_op(x, MPI.SUM) if USE_MPI else x
